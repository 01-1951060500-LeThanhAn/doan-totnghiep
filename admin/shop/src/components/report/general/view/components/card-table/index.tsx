...
export const columns: ColumnDef<ProductTableGeneral>[] = [
  {
    accessorKey: "code",
    header: "Mã sản phẩm",
    cell: ({ row }) => (
  ...
  },
  {
    accessorKey: "",
    header: "Có thể bán",
    cell: ({ row }: { row: Row<ProductTableGeneral> }) => {
      const total: number =
        (row.getValue("inventory_number") as number) -
        (row.getValue("pendingOrderQuantity") as number);
      return (
        <>
  ...
  {
    accessorKey: "pendingOrderQuantity",
    header: "Đang giao dịch",
    cell: ({ row }) => (
      <p className="capitalize">{row.getValue("pendingOrderQuantity")}</p>
    ),
  },
  {
    accessorKey: "pendingWarehouseQuantity",
    header: "Hàng đang về",
    cell: ({ row }) => (
      <p className="capitalize">{row.getValue("pendingWarehouseQuantity")}</p>
    ),
  },

  {
    accessorKey: "generalId",
    header: "Chi nhánh quản lý",
    cell: ({ table }) => {
      const { general, theme } = table.options.meta as Data;
      return (
...
type Props = {
  data: ProductTableGeneral[] | undefined;
  general: Results;
};

export default function GeneralDetailReportInventoriesView({
  data,
  general,
}: Props) {
